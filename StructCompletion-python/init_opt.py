...
def gaussian_kernel(ksize, sigma):
    kernel = np.zeros((ksize[0], ksize[1]), dtype=np.float64)
    halfsizeX = ksize[0] // 2
    halfsizeY = ksize[1] // 2
...
optS.lambdaCoherence = 1e-2*0.8           # Weighting parameter for coherence

# Planar cost
optS.lambdaPlane  = 5e-2*2.5               # Weighting parameters for planar cost

# Directional cost
optS.lambdaDirect = 5                 # Weighting parameters for directional cost
optS.directThres  = 0.05              # Directional cost threshold

optS.lambdaProx   = 0e-2+0.01            # Weighting parameters for proximity cost
optS.proxThres    = 0.25

optS.lambdaReg    = -0.02+0.01             # Weighting parameters for encouraging
